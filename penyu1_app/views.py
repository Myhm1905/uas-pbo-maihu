...
def beranda(request):
    ...
    klasifikasis = Klasifikasi.objects.all()
    data = {  
        'Title' : "UAS PBO",
...
@login_required(login_url='login')    
def update_klasifikasi(request, id_klasifikasi):
    klasifikasi = Klasifikasi.objects.get(id = id_klasifikasi)
    judul = "Update Data"
    template = "update_klasifikasi.html"

    if request.POST:
        form = FormKlasifikasi(request.POST, instance=Klasifikasi)
        if form.is_valid():
            form.save()
            pesan = "Data berhasil diupdate!"
            data = {
        ...
        form = FormKlasifikasi(instance=Klasifikasi)
        data = {
            'Title' : "UAS PBO",
            'Heading' : "Update Data",
            "form" : form,
...
@login_required(login_url='login')
def delete2(request, id_klasifikasi):
    klasifikasi = Klasifikasi.objects.get(id = id_klasifikasi)
    klasifikasi.delete()
